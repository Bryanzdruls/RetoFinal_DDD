...
public class Resource implements ValueObject<Map<String,?>> {
    ...
    private Resource(Integer quantity, ResourceEnum name){
        if (quantity != null && quantity >=0 && validateEnum(name)) {
            ...
            throw new IllegalArgumentException("Quantity must be at least zero and resource name must be one of those [WOOD,BRICK,SHEEP, WHEAT, ORE]" );
        }
    }
    public static Resource of(Integer quantity,  ResourceEnum name){
        return new Resource(quantity, name);
    }
